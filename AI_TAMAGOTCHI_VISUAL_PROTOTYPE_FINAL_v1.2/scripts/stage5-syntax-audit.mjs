...
const roots = ['src', 'mock-server', 'tests'];
const files = [];
function walk(dir) {
...
if (diagnostics.length) {
  for (const { file, d } of diagnostics) console.error(`${path.relative(root, file)}: ${ts.flattenDiagnosticMessageText(d.messageText, '\n')}`);
  process.exit(1);
}
console.log('STAGE5_TS_SYNTAX_AUDIT=PASS');
